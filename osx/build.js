...
var builder = {
	...
	buildLess : function(file) {
		...
		this._parser.parse(file, function (err, tree) {
			...
			_fs.writeFile(that.localPath+'/'+that.outputCss, that.css, function (err) {
				if (err) throw err;
				console.log(that.outputCss+' written !');
				_fs.chmod(that.localPath+'/'+that.outputCss, 0777, function(err) {
